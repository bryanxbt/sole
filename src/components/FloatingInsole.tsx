...
export function FloatingInsole({
  ...
  const rotate = useTransform(scrollYProgress, [0, 1], rotateOnScroll ? [-8, 12] : [0, 0]);
  const y = useTransform(scrollYProgress, [0, 1], rotateOnScroll ? [30, -30] : [0, 0]);
  const smoothRotate = useSpring(rotate, { stiffness: 60, damping: 20 });
  const smoothY = useSpring(y, { stiffness: 60, damping: 20 });

  return (
    <div ref={ref} className={`relative ${className}`} style={{ width: size, maxWidth: "100%" }}>
      <motion.div
        style={{
